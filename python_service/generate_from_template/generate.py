...
def replace_macros_in_word(doc_path, replacements, params, output_path):
...
async def process_document(file_key, data):
    ...
    head_template = data.get('head_template', "")
    ...
    replace_macros_in_word(
        "generate_from_template/templates/" + action_type + ".docx",
        replacements,
        params,
        output_path_word
    )
...
async def generate_from_template(request) -> str:
    """
    params: массив значений, которые будут подставляться в плейсхолдеры
    file_key: уникальный идентификатор файла
    """
