...
public:
  ...
  AddEmployeeResponse(const std::string& l, const std::string& p) : login(l), password(p) {}
...
class AddEmployeeHandler final : public userver::server::handlers::HttpHandlerBase {
 public:
  static constexpr std::string_view kName = "handler-v1-employee-add";

  AddEmployeeHandler(const userver::components::ComponentConfig& config,
  ...
  std::string HandleRequestThrow(
    ...
    auto result = pg_cluster_->Execute(
          ...
          "INSERT INTO working_day.employees(id, name, surname, patronymic, password) VALUES($1, $2, $3, $4, $5) "
          "ON CONFLICT (id) "
          "DO NOTHING",
          id, request_body.name, request_body.surname, request_body.patronymic, password);
