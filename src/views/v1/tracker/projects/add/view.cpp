...
class TrackerProjectsAddHandler final
 ...
 public:
  ...
  std::string HandleRequestThrow(
    ...
    if (!std::regex_match(request_body.project_key, pattern)) {
      request.GetHttpResponse().SetStatus(
          userver::server::http::HttpStatus::kBadRequest);
      return ErrorMessage{"Project key must contain only uppercase Latin letters"}.ToJsonString();
    }

    std::string project_id = request_body.project_key;
