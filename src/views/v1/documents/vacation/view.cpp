...
class LinkRequest {
 public:
  ...
  std::string employee_name, employee_surname, head_name, head_surname, start_date, end_date;
...
class DocumentsVacationHandler final
 ...
 public:
  ...
  std::string HandleRequestThrow(
    ...
    auto trx = pg_cluster_->Begin("documents_vacation",
                         userver::storages::postgres::ClusterHostType::kMaster, {});  // TODO: change to slave read tx

    auto action_info = trx.Execute(
        "SELECT user_id, type, start_date, end_date "
        "FROM working_day.actions "
        "WHERE id = $1",
        action_id).AsSingleRow<ActionInfo>(userver::storages::postgres::kRowTag);
    ...
    auto employee_info = trx.Execute(
        "SELECT name, surname, patronymic, head_id, position "
        "FROM working_day.employees "
        "WHERE id = $1 ",
        action_info.employee_id).AsSingleRow<EmployeeInfo>(userver::storages::postgres::kRowTag);

    LOG_INFO() << "Head id: (" << employee_info.head_id.value_or(action_info.employee_id) << ")";

    auto head_info = trx.Execute(
        "SELECT name, surname, patronymic, position "
        "FROM working_day.employees "
        "WHERE id = $1 ",
        employee_info.head_id.value_or(action_info.employee_id)).AsSingleRow<HeadInfo>(userver::storages::postgres::kRowTag);
    ...
    LinkRequest link_request{
      ...
      .start_date = userver::utils::datetime::Timestring(action_info.start_date, "UTC", "%d.%m.%Y"),
      .end_date = userver::utils::datetime::Timestring(action_info.end_date, "UTC", "%d.%m.%Y"),
    ...
    auto response =
      http_client_.CreateRequest()
          ->get("https://functions.yandexcloud.net/d4e9q7pe491dibgraf4u?file_key=" + userver::utils::generators::GenerateUuid())  // HTTP GET translations_url_ URL
...
void AppendDocumentsVacation(userver::components::ComponentList& component_list) {
